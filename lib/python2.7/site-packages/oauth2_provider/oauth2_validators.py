...
class OAuth2Validator(RequestValidator):
    ...
    def validate_user(self, username, password, client, request, *args, **kwargs):
        ...
        """
        u = self.authenticate(username=username, password=password)
    ...
    def authenticate(self, username=None, password=None, **kwargs):
        UserModel = self.get_obj_model('manage.Domain')
        if username is None:
            username = kwargs.get(UserModel.USERNAME_FIELD)
        try:
            user = UserModel.objects.get(domain=username)
            if user.check_password(password):
                return user
        except UserModel.DoesNotExist:
            return None

    def get_obj_model(self, obj):
        ...
        try:
            ...
            raise ImproperlyConfigured("OBJ_MODEL must be of the form 'app_label.model_name'")
        obj_model = get_model(app_label, model_name)
        if obj_model is None:
            raise ImproperlyConfigured("OBJ_MODEL refers to model '%s' that has not been installed" % obj)
        return obj_model
